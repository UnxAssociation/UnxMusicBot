...
async def custom_Emoji(ctx: Message):
    if ctx.forward_date:
        return (
            ""
            if ctx.forward_sender_name
...
async def pyrogram_to_quotly(messages, is_reply):
    ...
    for message in messages:
        ...
        if message.entities:
            ...
            the_message_dict_to_append["entities"] = []
        the_message_dict_to_append["chatId"] = await message_Sender_Id(message)
        ...
        the_message_dict_to_append["from"]["id"] = await message_Sender_Id(message)
        the_message_dict_to_append["from"]["name"] = await message_Sender_Name(
            message
        )
        the_message_dict_to_append["from"][
            "username"
        ] = await message_Sender_Username(message)
        the_message_dict_to_append["from"]["type"] = message.chat.type.name.lower()
        the_message_dict_to_append["from"]["photo"] = await message_Sender_Photo(
            message
        )
    ...
    re = await http.post("https://bot.lyo.su/quote/generate.png", json=payload)
    if not re.is_error:
        return re.read()
    else:
        raise SakuraException(re.json())
